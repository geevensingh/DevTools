...
def test_plan_layout_excludes_4conn_unreachable_pocket_from_polygon():
    """The calibration polygon is built from an 8-connectivity CC scan
    (``geometry.find_connected_components``), which can glue together
    two regions joined only by a single-pixel diagonal touch. The
    renderer's flood-fill is 4-connectivity, so it stops at the
    diagonal bridge and never colors the "pocket" past it. Without
    the polygon-vs-fill intersection introduced by passing ``image_bgr``,
    the layout planner can pick those unreachable pocket pixels as the
    office-number position — the rendered output then shows the number
    on a white background while the rest of the room is colored.

    This was the actual cause of the office-1180 visual bug on the
    Millennium B map: the door pocket was 8-conn-joined to the room
    body but the office number landed in the door pocket, which the
    renderer never colored.
    """
    # Build a 250x250 image with two square rooms touching diagonally.
    # Room A: white interior at cols/rows 6..99, enclosed by black walls
    # at row/col 5 and 100.
    # Room B: white interior at cols/rows 102..209.
    # Diagonal bridge: pixels (100, 100) and (101, 101) are white so the
    # 8-conn CC glues them, but 4-conn flood-fill from inside room A
    # stops at (100, 100) and cannot reach room B.
    ...
    img[101:211, 101] = 0
    img[101:211, 210] = 0
    # Punch the diagonal opening between rooms A and B.
    img[100, 100] = 255
    img[101, 101] = 255

    # Polygon mask = room A interior + room B interior + diagonal bridge.
    # (Mimics what 8-conn CC produces from the binarized image.)
    poly = np.zeros((250, 250), dtype=bool)
    poly[6:100, 6:100] = True       # room A interior
    poly[102:210, 102:210] = True   # room B interior
    poly[100, 100] = True
    poly[101, 101] = True

    room = Room(
        id=1,
        polygon_rle=mask_to_rle(poly),
        area_px=int(poly.sum()),
        # Bbox spans both rooms — corner candidates 2..5 in
        # ``_place_office_number_in_polygon`` would otherwise land in
        # room B (the unreachable pocket).
        bbox=(5, 5, 206, 206),
    )
    # Label sits in the unreachable pocket (room B) — this is the
    # crucial test setup. With the AND-with-fill-mask intersection,
    # ``_place_office_number_in_polygon``'s candidate #1 (centered on
    # the original label) lands in the pocket and is rejected, forcing
    # the planner to find a spot inside room A. Without the
    # intersection, the polygon still includes the pocket and the
    # number is placed there — exactly the Millennium B office-1180
    # bug. The fill_seed is in room A, so the renderer fills room A
    # only.
    label = Label(
        id="100",
        bbox=(148, 150, 16, 8),
    ...
    # Pass the image so plan_layout intersects with the 4-conn reachable
    # area.
    layout, issues = plan_layout(cal, [_asn("A B", "100")], image_bgr=img)
    assert [i for i in issues if i.severity == "error"] == []
    assert len(layout.entries) == 1
    entry = layout.entries[0]

    # The office number must land inside room A's reachable interior.
    nx, ny, nw, nh = entry.office_number.bbox
    cx = nx + nw // 2
    cy = ny + nh // 2
    assert 6 <= cx < 100 and 6 <= cy < 100, (
        f"office number centered at ({cx},{cy}); expected inside room A "
        f"(6..99, 6..99) — the only 4-conn-reachable area from seed (10,10). "
        f"Room B is at (102..209, 102..209) but is only diagonally connected "
        f"so the renderer will leave it white."
    )

    # Every planned name pixel must lie inside room A too.
    for name in entry.names:
        nbx, nby, nbw, nbh = name.bbox
        assert (nbx + nbw) <= 100 and (nby + nbh) <= 100, (
            f"name {name.rendered_text!r} bbox {name.bbox} extends into "
            f"room B (the 4-conn-unreachable pocket)"
        )
