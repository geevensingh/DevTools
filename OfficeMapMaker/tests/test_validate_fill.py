...
def test_cli_validate_fill_clean_run_returns_0(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch

    map_path, cal_path = _write_minimal_cal_and_map(tmp_path)
    args = build_parser().parse_args(
        ["validate", "fill", "--map", str(map_path), "--calibration", str(cal_path)]
    )
    assert dispatch(args) == 0
    overview = cal_path.with_name(cal_path.stem + "_rooms_overview.png")
...
def test_cli_validate_fill_missing_calibration_returns_2(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch
...
def test_cli_validate_fill_missing_map_returns_2(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch
