...
def test_cli_tile_writes_outputs_under_provided_out_dir(tmp_path, capsys):
    ...
    assert (out_dir / "contact_sheet.png").exists()
    # At least one tile.
    assert list(out_dir.glob("page-*.png"))


def test_cli_tile_warns_when_composite_not_reviewed(tmp_path, capsys):
    from officemapmaker.__main__ import main
    ...
    assert rc == 0
    assert "has not been confirmed" in captured.err


def test_cli_tile_returns_2_when_composite_missing(tmp_path, capsys):
    from officemapmaker.__main__ import main
    ...
    captured = capsys.readouterr()
    assert rc == 2
    assert "composite not found" in captured.err
