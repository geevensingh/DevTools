...
def test_cli_validate_labels_clean_run_returns_0(tmp_path: Path, capsys):
    ...
    # Calibration must reference the map_image filename, which load_calibration
    # then resolves alongside the calibration.json file.
    ...
    assert rc == 0
    # Review PNG should be written.
    assert (cal_path.with_name("calibration_validation_labels_review.png")).exists()


def test_cli_validate_labels_with_errors_returns_1(tmp_path: Path):
    from officemapmaker.__main__ import main
...
def test_cli_validate_labels_missing_calibration_returns_2(tmp_path: Path):
    from officemapmaker.__main__ import main
...
def test_cli_validate_labels_missing_assignments_returns_2(tmp_path: Path):
    from officemapmaker.__main__ import main
