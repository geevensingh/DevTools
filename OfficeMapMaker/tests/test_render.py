...
def test_cli_build_clean_run_returns_0_and_writes_outputs(synthetic_setup, tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch

    tmp_path, map_path, cal, cal_path = synthetic_setup
    asn_path = tmp_path / "people.csv"
    asn_path.write_text("Full Name,Office number,Team\nAlice Smith,100,BITS\nBob Jones,200,FPAA\n")

    layout_path = tmp_path / "layout.json"
    layout, _ = plan_layout(cal, [
        Assignment("Alice Smith", "100", "BITS", 2),
        Assignment("Bob Jones", "200", "FPAA", 3),
    ])
    ...
    assert rc == 0
    assert out_path.exists()
    assert (tmp_path / "composite_review.png").exists()
    assert (tmp_path / "composite.png.manifest.json").exists()
    assert not (tmp_path / "composite.png.reviewed").exists()


def test_cli_build_missing_inputs_returns_2(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch
    ...
    args = build_parser().parse_args([
        ...
        "--layout", str(tmp_path / "missing-layout.json"),
    ])
    assert dispatch(args) == 2


def test_cli_build_with_team_overrides(synthetic_setup, tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch
    ...
    layout_path = tmp_path / "layout.json"
    save_layout(
        plan_layout(cal, [Assignment("Alice", "100", "BITS", 2)])[0],
        layout_path,
    )
    teams_path = tmp_path / "teams.json"
    teams_path.write_text(json.dumps({"BITS": "#FFCCAA"}))
    ...
    composite = cv2.imread(str(out_path), cv2.IMREAD_COLOR)
    # #FFCCAA in BGR is (0xAA, 0xCC, 0xFF).
    assert (composite[200, 150] == (0xAA, 0xCC, 0xFF)).all()


def test_cli_build_stale_map_hash_blocks_render(synthetic_setup, tmp_path):
    """If layout.map_hash mismatches the current map, build refuses."""
    from officemapmaker.__main__ import build_parser, dispatch
    ...
    layout_path = tmp_path / "layout.json"
    layout, _ = plan_layout(cal, [Assignment("Alice", "100", "BITS", 2)])
    # Force a stale hash.
    layout.map_hash = "sha256:00000000"
    save_layout(layout, layout_path)
...
def test_cli_build_force_flag_overrides_stale_hash(synthetic_setup, tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch
...
def test_cli_build_confirm_writes_sentinel(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch

    composite_path = tmp_path / "composite.png"
    composite_path.write_bytes(b"\x89PNG\r\n")  # just needs to exist
    args = build_parser().parse_args([
        "build", "confirm", "--composite", str(composite_path),
    ])
    assert dispatch(args) == 0
    assert (tmp_path / "composite.png.reviewed").exists()


def test_cli_build_confirm_missing_returns_2(tmp_path):
    from officemapmaker.__main__ import build_parser, dispatch

    args = build_parser().parse_args([
        "build", "confirm", "--composite", str(tmp_path / "missing.png"),
    ])
    assert dispatch(args) == 2
